...
async function getDynamoDBMetrics(): Promise<{ geek_table: number, connection_table: number }> {
    const ddbClient = new DynamoDBClient({ region: process.env.AWS_REGION });
    const g = await ddbClient.send(new DescribeTableCommand({ TableName: GEEK_TABLE }));
    const c = await ddbClient.send(new DescribeTableCommand({ TableName: CONNECTION_TABLE }));
    ddbClient.destroy();
    return { geek_table: g.Table.ItemCount, connection_table: c.Table.ItemCount };
}
...
export async function handler(event: { Payload: CloudWatchPayload }): Promise<void | HttpResponse> {
    ...
    const { geek_table, connection_table } = await getDynamoDBMetrics();
